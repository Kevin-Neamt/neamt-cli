...
def install_cmd(source: str = typer.Argument(..., help="GitHub URL or local path")) -> None:
    """Install a skill."""
    from neamt.cli.commands.install import install
    install(source)
...
def uninstall_cmd(skill_id: str = typer.Argument(...)) -> None:
    """Uninstall a skill."""
    from neamt.cli.commands.uninstall import uninstall
    uninstall(skill_id)
...
def list_cmd() -> None:
    ...
    from neamt.cli.commands.list import list_skills
    list_skills()
...
def update_cmd(skill_id: str = typer.Argument(...)) -> None:
    ...
    from neamt.cli.commands.update import update
    update(skill_id)
...
def info_cmd(skill_id: str = typer.Argument(...)) -> None:
    """Show skill details."""
    from neamt.cli.commands.info import info_cmd as _info
    _info(skill_id)
...
def enable_cmd(skill_id: str = typer.Argument(...)) -> None:
    ...
    from neamt.cli.commands.toggle import enable
    enable(skill_id)
...
def disable_cmd(skill_id: str = typer.Argument(...)) -> None:
    ...
    from neamt.cli.commands.toggle import disable
    disable(skill_id)
...
def start_cmd(
    ...
    from neamt.cli.commands.server import start
    start(host, port)
...
def stop_cmd() -> None:
    ...
    from neamt.cli.commands.server import stop
    stop()
...
def restart_cmd(
    ...
    from neamt.cli.commands.server import restart
    restart(host, port)
...
def config_cmd(
    ...
    from neamt.cli.commands.config import config_get, config_list, config_set
    if action == "list":
        config_list()
    elif action == "get":
        config_get(key)
...
def doctor_cmd() -> None:
    ...
    from neamt.cli.commands.doctor import doctor
    doctor()
